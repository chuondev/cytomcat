...
import com.chuonye.tomcat.container.Valve;
import com.chuonye.tomcat.container.servletx.Request;
import com.chuonye.tomcat.container.servletx.Response;
import com.chuonye.tomcat.util.Escape;
...
public class ErrorReportValve extends Valve {
    ...
    protected void report(Request request, Response response, Throwable throwable) {
     ...
     // Do nothing if there is no report for the specified status code
        ...
        sb.append("cytomcat - Error report");
        sb.append("</title>");
    ...
    protected String getPartialServletStackTrace(Throwable t) {
        ...
        for (int i = elements.length - 1; i >= 0; i--) { // 找到容器内部堆栈开始位置
            if ((elements[i].getClassName().startsWith("com.chuonye.tomcat.container.servletx.AppFilterChain"))
        ...
        for (int i = 0; i < pos; i++) { // 排除容器内部堆栈信息
            if (!(elements[i].getClassName().startsWith("com.chuonye.tomcat.container."))) {
